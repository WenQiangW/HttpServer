...
static void show_501(int fd)
{
	const char *head_line = "HTTP/1.0 500 Method Not Implemented\r\n";
	send(fd,head_line,strlen(head_line),0);
	const char *blank_line = "\r\n";
	send(fd,blank_line,strlen(head_line),0);
	const char* msg = "<html><h1>Method Not Implemented!</h1></html>\r\n";
	send(fd,msg,strlen(msg),0);
}
